...
export default function SellPage() {
	const [activeCat, setActiveCat] = React.useState<Category | "ทั้งหมด">(
		"ทั้งหมด"
	);
	...
	const [variantDlg, setVariantDlg] = React.useState<{
		p: Product | null;
		v: string;
	}>({p: null, v: ""});
	...
	React.useEffect(() => {
		...
		if (!base) {
			setErrorMsg(
				"ยังไม่ได้ตั้งค่า NEXT_PUBLIC_API_BASE ใน .env"
			);
			setLoading(false);
			return;
		}
	...
	const categories = React.useMemo(() => {
		...
		if (activeCat !== "ทั้งหมด" && !set.has(activeCat))
